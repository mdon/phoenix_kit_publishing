...
1. **Internal-prefix scope.** Publishing's catch-all is registered under `/__phoenix_kit_publishing_dispatch/...` instead of `/`. Core's `phoenix_kit_routes/0` macro emits this scope with the standard `:browser` + `:phoenix_kit_*` pipelines plus an extra `:phoenix_kit_publishing_internal` pipeline that runs `RouterDispatch.restore_path/2`.
